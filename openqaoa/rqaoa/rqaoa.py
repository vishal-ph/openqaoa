...
def final_solution(max_terms_and_stats_list: list, classical_results: dict, hamiltonian: Hamiltonian):
    """
    ...
    # Extract the states of results from classical solution
    cl_states = classical_results['opt states']
...
def adaptive_rqaoa(hamiltonian: Hamiltonian,
    ...
    if type(mixer['connectivity']) == list:
        raise NotImplementedError(f'Custom mixer connectivities are not currently supported')
    else:
        mixer_hamiltonian = get_mixer_hamiltonian(n_qubits,mixer['type'],mixer['connectivity'],mixer['coeffs'])
